...
cmake --build "${BUILD_DIR}" --target ipc_app freetype_bitmap_demo audio_capture_opus_demo audio_capture_apm_pcm_demo audio_opus_playback_demo audio_playback_manager_demo audio_apm_gain_demo audio_apm_aec_smoke_demo audio_mixer_inspect_demo --parallel "$(nproc)"

if [[ -x "${BASE}/tools/update_compile_commands.sh" ]]; then
    "${BASE}/tools/update_compile_commands.sh" || true
fi

echo "=== built RV1126B targets (ipc_app, freetype_bitmap_demo, audio_capture_opus_demo, audio_capture_apm_pcm_demo, audio_opus_playback_demo, audio_playback_manager_demo, audio_apm_gain_demo, audio_apm_aec_smoke_demo, audio_mixer_inspect_demo): ${BUILD_DIR} ==="
