...
# 编译；IPC 推流服务与 OSD demo 不依赖该路径。这里显式构建板端当前支持的两个目标。
cmake --build "${BUILD_DIR}" --target ipc_app freetype_bitmap_demo --parallel "$(nproc)"

if [[ -x "${BASE}/tools/update_compile_commands.sh" ]]; then
    "${BASE}/tools/update_compile_commands.sh" || true
fi

echo "=== built RV1126B targets (ipc_app, freetype_bitmap_demo): ${BUILD_DIR} ==="
